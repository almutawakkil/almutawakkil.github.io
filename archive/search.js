...
async function init(){
  try{
    const r=await fetch("data/archive.json",{cache:"no-store"});
    ...
    $("#status").textContent="Could not load data/archive.json. Run the Python archiver first, then serve this folder through a local web server.";
    console.error(e);
  }
}
